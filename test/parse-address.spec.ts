...
describe('Address Parser (parseAddress)', function () {
    ...
    describe('Partial Addresses', () => {
        it('City, State ZIP', () => {
            const result = addresser.parseAddress('Charlotte, NC 28202', { allowPartialAddress: true });
            expect(result.formattedAddress).to.equal('Charlotte, NC');
            expect(result.id).to.equal('charlotte-nc');
            expect(result.stateAbbreviation).to.equal('NC');
