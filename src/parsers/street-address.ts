...
const usStreetDirectional: { [key: string]: string } = {
  north: "N",
  northeast: "NE",
  east: "E",
  southeast: "SE",
...
export const parseStreetAddress = (streetString: string | undefined): ParseStreetAddressResult => {
  ...
  if (match) {
    ...
    if (fullStreetName && /\w\s+ave(?:nue)?.?\s\w/i.test(fullStreetName)) {
      resultStreetName = replaceCaseInsensitive(fullStreetName, "avenue", true, "Ave")?.replace(
        /\./g,
        "",
      );
    } else {
      ...
      const direction = Object.keys(usStreetDirectional).find((key) => {
        return (
          uppercaseLine1.endsWith(" " + usStreetDirectional[key]) ||
      ...
      if (streetParts.length > 2) {
        ...
        const streetType = usStreetTypes[lowercaseStreetType as keyof typeof usStreetTypes];
