...
export const parseUsAddress = function (address: string, options?: Options): Partial<ParsedAddress> {
    if (!address) {
    ...
    if (addressString && resultPlaceName && resultStateAbbreviation) {
        resultFormattedAddress = makeFormattedAddress(
            resultStateAbbreviation,
            resultPlaceName,
            addressString,
            resultZipCode,
...
export const makeFormattedAddress = (stateCode: string, placeName: string, line1?: string, zipCode?: string): string => {
    return (line1 ? (line1 + ", ") : '') + placeName + ", " + stateCode + (zipCode ? (" " + zipCode) : '');
};

export const makeAddressId = (formattedAddress: string): string => {
