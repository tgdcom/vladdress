...
import usStreetTypes from "../data/us/street-types.json";
export const streetAddressRegex = /^(?<streetNum>(?:\d+\w*|\w*\d+)?)?\s*(?<streetName>.+)$/i;
export const matchesStreetAddress = (input) => {
    return !!input && streetAddressRegex.test(input);
};
export const matchStreetAddress = (input) => {
...
export const parseStreetAddress = (streetString) => {
    if (!streetString) {
        throw new Error("No Street String Passed to parseStreetAddress");
    }
    const match = matchStreetAddress(streetString);
    let resultStreetName;
    let resultAddressLine1;
    let resultAddressLine2;
    let resultStreetDirection;
    let resultStreetSuffix;
    let resultStreetNumber;
    if (match) {
        ...
        if (fullStreetName && /\w\s+ave(?:nue)?.?\s\w/i.test(fullStreetName)) {
            ...
            const direction = Object.keys(usStreetDirectional).find((key) => {
                return (uppercaseLine1.endsWith(" " + usStreetDirectional[key]) ||
                    uppercaseLine1.endsWith(" " + key.toUpperCase()));
            });
            if (direction) {
            ...
            if (lowercaseStreetName && !resultStreetSuffix) {
                ...
                if (foundEnding) {
                    resultStreetName = resultStreetName
                        ?.substring(0, resultStreetName.length - foundEnding.length)
                        .trim();
                }
            }
        }
        resultStreetName = toTitleCase(resultStreetName || "").replace("Fm", "FM");
        resultStreetNumber = match.groups?.streetNum;
        resultAddressLine1 = [
            resultStreetNumber,
            resultStreetName,
            resultStreetSuffix,
            resultStreetDirection,
        ]
            .filter((s) => !!s)
            .join(" ");
    }
